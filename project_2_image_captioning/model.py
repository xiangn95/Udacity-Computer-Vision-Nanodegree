...
#         self.init_hidden_state()
    ...
    def forward(self, features, captions):
        ...
        x = torch.cat((features.unsqueeze(1), caption_embeds), dim=1) 
        x, _ = self.lstm(x)
        x = self.fc(x)
...
#         self.fc.weight.data.uniform_(-0.1, 0.1)
    ...
    def sample(self, features, states=None, max_len=20):
        ...
        word_list = [0]
        words = torch.zeros((1, 1), dtype=torch.long, device=device)
        
        words = self.embedding(words)
        
        x = torch.cat((features, words), dim=1) 
#         x = torch.cat((features.unsqueeze(1), caption_embeds), dim=1) 
        for i in range(max_len-1):            
            ...
            word_list.append(next_word.item())
            
            if next_word.item() == 1:
                break
            
            
            next_word = self.embedding(next_word)
#             set_trace()
            words = torch.cat((words, next_word), dim=1)
#             set_trace()
            x = torch.cat((features, words), dim=1)
